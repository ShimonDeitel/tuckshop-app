...
struct ContentView: View {
    ...
    @State private var editingEntry: TuckshopEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No entries yet")
                ...
                .foregroundStyle(Theme.textPrimary)
            Text("Track allowance given and spent for each of your kids.")
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
    ...
    private func row(for entry: TuckshopEntry) -> some View {
        HStack {
...
struct EntryEditView: View {
    ...
    var onSave: (TuckshopEntry) -> Void

    private let existingID: UUID?

    init(store: Store, existing: TuckshopEntry? = nil, onSave: @escaping (TuckshopEntry) -> Void) {
        self.store = store
        self.onSave = onSave
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let entry = TuckshopEntry(id: existingID ?? UUID(), title: title.isEmpty ? "Untitled" : title, amount: amount, date: date, note: note)
                        onSave(entry)
                        dismiss()
                    }
